...
namespace LuaPlayer
{
    ...
    {
        Eluna::Push(L, player->CanUninviteFromGroup() == PARTY_RESULT_OK);
        return 1;
    }
    ...
    int GetGuildName(Eluna* /*E*/, lua_State* L, Player* player)
    {
        if (!player->GetGuildId())
            return 1;
        Eluna::Push(L, sObjectMgr.GetGuildNameById(player->GetGuildId()));
        return 1;
    }
    ...
    {
        Eluna::Push(L, sObjectMgr.GetGuildById(player->GetGuildId()));
        return 1;
    }
    ...
    int UnbindInstance(Eluna* /*E*/, lua_State* L, Player* player)
    {
        ...
        if (difficulty < TOTAL_DIFFICULTIES)
            player->UnbindInstance(map, difficulty);
        return 0;
    }
    ...
    int AddItem(Eluna* /*E*/, lua_State* L, Player* player)
    {
        ...
        ItemPosCountVec dest;
        uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, itemCount, &noSpaceForCount);
        if (msg != EQUIP_ERR_OK)
    ...
    int GroupInvite(Eluna* /*E*/, lua_State* L, Player* player)
    {
        ...
        if (group)
            success = !group->IsFull() && group->AddInvite(invited, false);
        else
        {
            // Create new group if one not found
            group = new Group;
            success = group->AddLeaderInvite(player) && group->AddInvite(invited, true);
            if (!success)
                delete group;
        }
    ...
    int GroupCreate(Eluna* /*E*/, lua_State* L, Player* player)
    {
        ...
        if (!group->AddMember(invited->GetGUID(), invited->GetName()))
            return 0;
        Eluna::Push(L, group);
        return 1;
    }
